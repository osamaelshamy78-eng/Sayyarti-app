...
class AppErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) { console.error("Sayyarti startup error:", error, info); }
  render() {
    if (this.state.error) {
      const message = this.state.error?.message || "Unknown application error";
      return <div style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"#14171C",color:"#F2ECDD",padding:24,fontFamily:"system-ui,sans-serif"}}><div style={{maxWidth:560,textAlign:"center"}}><h1>Sayyarti</h1><p>The app could not start.</p><p dir="ltr" style={{color:"#F5B942",fontSize:13,wordBreak:"break-word"}}>{message}</p><button onClick={()=>window.location.reload()} style={{marginTop:16,padding:"10px 18px",borderRadius:8,border:0}}>Reload</button></div></div>;
    }
    return this.props.children;
  }
}
function StartupFallback() { return <div style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"#14171C",color:"#F2ECDD",fontFamily:"system-ui,sans-serif"}}>Loading Sayyarti…</div>; }
