...
const ISSUES = {
  ...
  commonFuses: {
    en: {
      ...
      causes: [
        "Fuse numbers and positions differ between car makes and models — always check the diagram printed on the inside of your fuse box cover or in the owner's manual before pulling any fuse",
        "There are usually two fuse boxes: one under the dashboard (cabin) and one under the hood (engine bay)",
      ],
      steps: [
        "Engine Control Unit (ECU) fuse — powers the main engine computer that runs the engine",
        "Fuel pump fuse — powers the pump that sends fuel from the tank to the engine",
        "Ignition/Starter fuse — powers the ignition switch and starter relay",
        "Main/Battery fuse — protects the main power feed from the battery",
        "Headlights fuse — powers the low/high beam headlights",
        "Tail & parking lights fuse — powers the rear lights and side parking lights",
        "Brake lights fuse — powers the lights that come on when you press the brake pedal",
        "Turn signal & hazard fuse — powers the indicators and hazard flashers",
        "Horn fuse — powers the horn",
        "Power windows fuse — powers the electric window motors",
        "Power door locks fuse — powers the central locking system",
        "Radio/infotainment fuse — powers the screen, radio, and USB/AUX ports",
        "AC & blower motor fuse — powers the AC compressor clutch and the cabin fan",
        "Wipers fuse — powers the windshield wiper motor",
        "12V socket / cigarette lighter fuse — powers the 12V charging socket",
        "Interior/dome light fuse — powers the cabin ceiling lights",
        "ABS fuse — powers the anti-lock braking system module",
        "Airbag (SRS) fuse — powers the airbag control module — never remove this one to test, get a mechanic if it's the suspected issue",
    ...
    ar: {
      ...
      causes: [
        "أرقام ومواقع الفيوزات بتختلف من ماركة لموديل لآخر، لازم دايمًا تشوف الرسم البياني المطبوع جوه غطاء علبة الفيوزات أو في كتيب السيارة قبل ما تطلع أي فيوز",
        "غالبًا في علبتين فيوزات: واحدة تحت الديكور جوه العربية، وواحدة تحت الكبوت",
      ],
      steps: [
        "فيوز كمبيوتر المحرك (ECU) — بيغذي كمبيوتر المحرك اللي بيتحكم في تشغيله",
        "فيوز طرمبة البنزين — بيغذي الطرمبة اللي بتوصل الوقود من الخزان للمحرك",
        "فيوز الكونتاكت/السلف — بيغذي مفتاح الكونتاكت وريلاي السلف",
        "الفيوز الرئيسي/فيوز البطارية — بيحمي التغذية الرئيسية من البطارية",
        "فيوز الأنوار الأمامية — بيغذي أنوار الشورت والفول",
        "فيوز الأنوار الخلفية وأنوار الوقوف — بيغذي الأنوار الخلفية والجانبية",
        "فيوز أنوار الفرامل — بيغذي الأنوار اللي بتضيء عند الضغط على الفرامل",
        "فيوز الإشارة والفلاشر — بيغذي إشارات اللف وأنوار الخطر",
        "فيوز الكلاكس — بيغذي الكلاكس",
        "فيوز الشبابيك الكهربائية — بيغذي موتورات الشبابيك",
        "فيوز قفل الأبواب المركزي — بيغذي نظام القفل المركزي",
        "فيوز الراديو ونظام الترفيه — بيغذي الشاشة والراديو ومنافذ اليو إس بي",
        "فيوز المكيف والمروحة — بيغذي كلتش الكمبروسر ومروحة التكييف الداخلية",
        "فيوز المساحات — بيغذي موتور مساحات الزجاج",
        "فيوز منفذ الشاحن/الولاعة — بيغذي منفذ الشحن ١٢ فولت",
        "فيوز إضاءة السقف الداخلية — بيغذي لمبات السقف جوه العربية",
        "فيوز نظام ABS — بيغذي وحدة نظام الفرامل المانعة للانزلاق",
        "فيوز الإيرباج (SRS) — بيغذي وحدة التحكم في الوسائد الهوائية، متشيلوش أبدًا للتجربة، لو هو المشتبه فيه لازم فني متخصص",
