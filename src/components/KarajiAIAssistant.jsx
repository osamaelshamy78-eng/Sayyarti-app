...
export default function KarajiAIAssistant({ lang = "ar" }) {
  ...
  const openDiagnose = () => {
    const target = document.querySelector("button") && Array.from(document.querySelectorAll("button")).find((b) => /diagnos|شخّص|عطل|fix/i.test(b.textContent || ""));
  ...
  return (
    <div dir={isAr ? "rtl" : "ltr"} style={{ position: "fixed", right: 14, bottom: 78, zIndex: 9998, fontFamily: "system-ui, sans-serif" }}>
      {open && (
        <div style={{ width: "min(360px, calc(100vw - 28px))", maxHeight: "min(650px, calc(100vh - 150px))", overflowY: "auto", background: C.panel, border: `1px solid ${C.line}`, borderRadius: 18, boxShadow: "0 18px 50px rgba(0,0,0,.45)", padding: 14, marginBottom: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label="Close" style={{ background: "transparent", border: `1px solid ${C.line}`, color: C.dim, borderRadius: 9, width: 32, height: 32 }}>×</button>
          </div>
          ...
          <div style={{ background: C.asphalt, border: `1px solid ${C.line}`, borderRadius: 13, padding: 12, marginBottom: 10 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <label style={{ color: C.dim, fontSize: 10 }}>
          ...
          <div style={{ border: `1px solid ${assessment.urgent ? C.red : C.green}55`, background: `${assessment.urgent ? C.red : C.green}0D`, borderRadius: 13, padding: 12, marginBottom: 10 }}>
            ...
            {assessment.reasons.length > 0 && <ul style={{ margin: "7px 0 0", paddingInlineStart: 18, color: C.dim, fontSize: 10.5, lineHeight: 1.55 }}>{assessment.reasons.slice(0, 3).map((reason) => <li key={reason}>{isAr ? reason.replace("High mileage", "عداد مرتفع").replace("Higher mileage", "عداد مرتفع").replace("A critical warning light requires prompt inspection.", "لمبة تحذير خطيرة تحتاج فحصًا سريعًا.").replace("A warning light is active; diagnose it before the issue grows.", "هناك لمبة تحذير؛ شخّصها قبل أن تتفاقم المشكلة.").replace("A reported symptom should be checked rather than ignored.", "الأعراض المبلغ عنها تحتاج فحصًا ولا يُفضّل تجاهلها.") : reason}</li>)}</ul>}
          </div>
          ...
          <div style={{ color: C.dim, fontSize: 9.5, lineHeight: 1.45, marginTop: 9, textAlign: "center" }}>{isAr ? "المؤشر تقديري وليس تشخيصًا ميكانيكيًا. لا تعتمد عليه في الحالات الخطرة." : "This indicator is a planning aid, not a mechanical diagnosis. Do not rely on it for safety-critical issues."}</div>
        </div>
      )}
